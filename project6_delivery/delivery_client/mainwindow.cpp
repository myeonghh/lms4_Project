...
void MainWindow::send_user_info(int act_type, int client_type, QString id = "", QString pw = "", QString phone_num = "")
{
    if(m_socket)
    {
        if(m_socket->isOpen())
        {
            ...
            switch (act_type) {
            case LOGIN:
                header.prepend(QString("fileType:login,sender:%1,senderid:%2,receiver:null,recieverid:null;").arg(client_type_to_string(client_type), id).toUtf8());
                break;
            case SIGNUP:
                header.prepend(QString("fileType:signUp,sender:%1,senderid:%2,receiver:null,recieverid:null;").arg(client_type_to_string(client_type), id).toUtf8());
                break;
            case IDDUPCHK:
                header.prepend(QString("fileType:idDupChk,sender:%1,senderid:%2,receiver:null,recieverid:null;").arg(client_type_to_string(client_type), id).toUtf8());
                break;
            case PNUMDUPCHK:
                header.prepend(QString("fileType:pNumDupChk,sender:%1,senderid:%2,receiver:null,recieverid:null;").arg(client_type_to_string(client_type), id).toUtf8());
                break;
            default:
                break;
            }
